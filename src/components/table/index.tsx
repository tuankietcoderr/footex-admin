...
function Table<T = any>({
    ...
    if (headers.length !== columns.length) throw new Error("Headers and columns must have the same length")
    const renderHeaders = () => {
    ...
    const renderColumns = (row: T, rowIndex: number) => {
    ...
    return (
        <table className={cn("w-full text-center text-sm text-gray-700 rtl:text-right dark:text-gray-400", className)}>
            ...
            <tbody>
                {data.length > 0 ? (
                    data.map((row, rowIndex) => (
                        <tr
                            key={rowIndex}
                            className='border-b bg-white hover:bg-accent dark:border-gray-700 dark:bg-gray-800'
                        >
